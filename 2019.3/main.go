...
func move(wire string, arr *[25000][25000]string, cmd string, startX, startY int) (int, int) {
	x, y := moveDecision(cmd)
	...
	for i := 1; i <= n; i++ {
		...
		if arr[moveY][moveX] != "" && arr[moveY][moveX] != wire {
			arr[moveY][moveX] = "+"
			intersections = append(intersections, []int{moveX, moveY})
		} else {
			arr[moveY][moveX] = wire
		}
	}

	return startX + n*x, startY + n*y
}

func moveDecision(cmd string) (x, y int) {
	switch cmd[0] {
	case 'R':
		x, y = 1, 0
	case 'L':
